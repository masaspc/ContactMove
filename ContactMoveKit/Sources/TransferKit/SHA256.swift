...
/// SHA-256 ハッシュ。Darwin では CryptoKit、それ以外(Linux の swift test)では
/// 純 Swift 実装(FIPS 180-4)を使う。両者は同一の結果を返す(テストで検証)。
enum SHA256Hasher {
    ...
    static func hexDigest(_ data: Data) -> String {
        ...
        return CryptoKit.SHA256.hash(data: data)
            ...
            .joined()
        #else
        return PureSHA256.digest([UInt8](data))
            .map { hexByte($0) }
            .joined()
        #endif
    }
    ...
    private static func hexByte(_ byte: UInt8) -> String {
        ...
        let high = table[table.index(table.startIndex, offsetBy: Int(byte >> 4))]
        let low = table[table.index(table.startIndex, offsetBy: Int(byte & 0x0F))]
        return String([high, low])
    }
}

/// 純 Swift の SHA-256 実装(FIPS 180-4)。CryptoKit が使えない環境向け。
enum PureSHA256 {
